...
def test_extract_source_payload_reconciles_reversed_dividend_branch_order_without_mismatching_conditions(
    monkeypatch,
):
    config = llm_document_extractor.LLMInputParserConfig(
        api_key="test-key",
    ...
    rules = payload["parsed_articles"][0]["paragraphs"][0]["extracted_rules"]
    assert [rule["rate"] for rule in rules] == ["10%", "5%"]
    assert rules[0]["conditions"] == ["In all other cases."]
    assert rules[1]["conditions"] == [
        "If the beneficial owner is a company which holds directly at least 25 per cent of the capital of the company paying the dividends."
    ]


def test_extract_source_payload_promotes_rate_bearing_rule_over_leading_narrative_rule(
    monkeypatch,
):
    config = llm_document_extractor.LLMInputParserConfig(
        api_key="test-key",
                                            ...
                                            "paragraphs": [
                                                {
                                                    ...
                                                    "source_reference": "Article 12(2)",
                                                    "text": (
                                                        "However, such royalties may also be taxed in the first-mentioned State, "
                                                        "but the tax so charged shall not exceed 10 per cent of the gross amount "
                                                        "of the royalties."
                                                    ),
                                                    "rules": [
                                                        {
                                                        ...
                                                        },
                                                        {
                                                            ...
                                                            "direction": "payer_to_payee",
                                                            "conditions": [
                                                                "Treaty cap applies if the payment qualifies for treaty treatment."
                                                            ],
                                                            "review_reason": "Source-state cap paragraph.",
    ...
    payload = llm_document_extractor.extract_source_payload_from_text(
        raw_text=(
            "Article 12 Royalties\n"
            "2. However, such royalties may also be taxed in the first-mentioned State, but the tax so charged "
    ...
    rules = payload["parsed_articles"][0]["paragraphs"][0]["extracted_rules"]
    assert rules[0]["rule_type"] == "taxation_right"
    assert rules[0]["is_primary_candidate"] is False
    assert rules[0]["rate"] == "N/A"
    assert rules[1]["rule_type"] == "source_tax_limit"
    assert rules[1]["is_primary_candidate"] is True
    assert rules[1]["rate"] == "10%"
